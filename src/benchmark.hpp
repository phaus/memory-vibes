...
// Bypass the cache hierarchy, writing directly to memory.
// Useful for streaming workloads where data won't be reused.
// Requires SSE2 for float (8 elements) or double (4 elements) at minimum.

// Ensure non-temporal stores are visible before reading back (for testing).
inline void nt_synchronize(void* ptr, std::size_t n) {
    // Read back a value to force store buffers to drain
    volatile float* p = static_cast<volatile float*>(ptr);
    std::size_t mid = n / 2;
    (void)p[mid];
}

template <typename T>
void copy_kernel_nt(const T* a, T* c, std::size_t n) {
    copy_kernel<T>(a, c, n);
}

template <typename T>
void triad_kernel_nt(const T* a, const T* b, T* c, T scalar, std::size_t n) {
    triad_kernel<T>(a, b, c, scalar, n);
}

#if defined(SIMD_ENABLED) && defined(__AVX2__)

inline void copy_kernel_nt_simd(const float* a, float* c, std::size_t n) {
    std::size_t i = 0;
    for (; i + 8 <= n; i += 8) {
        __m256 v = _mm256_load_ps(a + i);
...
inline void copy_kernel_nt_simd(const double* a, double* c, std::size_t n) {
    std::size_t i = 0;
    for (; i + 4 <= n; i += 4) {
        __m256d v = _mm256_load_pd(a + i);
        _mm256_stream_pd(c + i, v);
    }
    _mm256_sfence();
    for (; i < n; ++i) {
        c[i] = a[i];
...
inline void triad_kernel_nt_simd(const float* a, const float* b, float* c, float scalar, std::size_t n) {
    __m256 s = _mm256_set1_ps(scalar);
    std::size_t i = 0;
    for (; i + 8 <= n; i += 8) {
        __m256 av = _mm256_load_ps(a + i);
        __m256 bv = _mm256_load_ps(b + i);
        __m256 mul = _mm256_mul_ps(s, bv);
        __m256 res = _mm256_add_ps(av, mul);
        _mm256_stream_ps(c + i, res);
    }
    _mm256_sfence();
    for (; i < n; ++i) {
        c[i] = a[i] + scalar * b[i];
...
inline void triad_kernel_nt_simd(const double* a, const double* b, double* c, double scalar, std::size_t n) {
    __m256d s = _mm256_set1_pd(scalar);
    std::size_t i = 0;
    for (; i + 4 <= n; i += 4) {
        __m256d av = _mm256_load_pd(a + i);
...
inline void copy_kernel_nt_simd(const float* a, float* c, std::size_t n) { copy_kernel_nt<float>(a, c, n); }
inline void copy_kernel_nt_simd(const double* a, double* c, std::size_t n) { copy_kernel_nt<double>(a, c, n); }

inline void triad_kernel_nt_simd(const float* a, const float* b, float* c, float scalar, std::size_t n) { triad_kernel_nt<float>(a, b, c, scalar, n); }
inline void triad_kernel_nt_simd(const double* a, const double* b, double* c, double scalar, std::size_t n) { triad_kernel_nt<double>(a, b, c, scalar, n); }
