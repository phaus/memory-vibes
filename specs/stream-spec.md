...
5. **Reporting** – results must be reported in the standard CSV-friendly format:
   ...
   ALU        ...         ...       ...
   ```
   Note: Scale and Add kernels are defined in `benchmark.hpp` but are not run by the default benchmark path. The default execution runs Copy, Triad, RandomRW, and optionally ALU (with `-A`/`--alu` flag).
